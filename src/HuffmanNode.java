 ...
 * <p>
 * Each node may store data (if a leaf node) or the data field might be null (for internal nodes).
 * <p>
 * When building a Huffman tree, it is handy to track to the counts of each node's contained value or the sum of the
 * values of its subtrees. This count value defaults to 0. HuffmanNodes can be sorted based on this count value, with
 * smaller counts coming before larger. This count is not self-managing, so it should be updated manually if any of the
...
public class HuffmanNode<E> implements Comparable<HuffmanNode<E>> {
  ...
  /**
   * Returns a multi-line string showing this node as the root of a tree. Each node is on its own line, displayed as per
   * {@link #toString()}, but preceded by the given prefix. The prefix is extend by "&lt;" for each recursive call on a
   * left child and by "&gt;" for each call to a right child.
   */
